...
function GameOver:enter(_, playerSprite)
    self.image = playerSprite

    local g = anim8.newGrid(64, 64, self.image:getWidth(), self.image:getHeight())
    self.animationOver = anim8.newAnimation(g('1-6',21), 0.09, 'pauseAtEnd')
end
...
function GameOver:draw()
    ...
    self.animationOver:draw(self.image, 350, 100)

    love.graphics.setColor( 0, 1, 0.1, light.alpha)

    love.graphics.setFont(title)
    love.graphics.print("Game Over", 200, 150)
